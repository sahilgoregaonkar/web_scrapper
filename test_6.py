...
# Function to process subsections for each section, supporting multiple levels
def process_subsections(tag, processed_subsections):
    subsections_found = False
    subsection_data = []
    subsection_title = None
    subsection_content = None

    for subsection_tag in tag.find_all_next(['h1', 'h2', 'h3', 'h4', 'p']):  # Add more tag types
        # Look for section titles, which are usually marked with specific styles or tags
        if subsection_tag.name in ['h1', 'h2', 'h3', 'h4']:  
            subsection_title = subsection_tag.get_text(strip=True)  # Extract subsection title

            # After the subsection title, extract the content (usually in <p> tags)
            next_p_tag = subsection_tag.find_next('p')
            if next_p_tag:
                subsection_content = next_p_tag.get_text(strip=True)

            # Generate a unique identifier for this subsection
            subsection_key = subsection_title.strip()
            
            # Avoid duplicates
            if subsection_key not in processed_subsections:
                processed_subsections.add(subsection_key)

                # Try to get the subsection URL (if applicable)
                subsection_url = None
                try:
    ...
    return subsections_found, subsection_data
            ...
            for chapter_tag in soup.find_all(attrs={"data-chapter-title": True}):  # Look for elements with data-chapter-title
                ...
                for tag in chapter_tag.find_all_next(['h1', 'p']):
                    ...
                    if tag.name == 'h1' and 'level1' in tag.get('class', []):  # It's a section
                        ...
                        subsections_found, subsection_data = process_subsections(tag, processed_sections)

                        if subsections_found:
                            section_data["subsections"] = subsection_data
