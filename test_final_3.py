                    ...
                    if tag.name == 'h1' and 'level1' in tag.get('class', []):  # It's a section
                        ...
                        for subsection_tag in tag.find_all_next(['h1', 'p']):
                            ...
                            if subsection_tag.name == 'h1' and 'level2' in subsection_tag.get('class', []):  # It's a subsection
                                ...
                                # Look for deeper subsections (e.g., 102.2.1 -> level3)
                                for deeper_tag in subsection_tag.find_all_next(['h1', 'p']):
                                    ...
                                    if deeper_tag.name == 'h1' and 'level3' in deeper_tag.get('class', []):  # It's a deeper subsection (level3)
                                        ...
                                        # Try to get the deeper subsection URL
                                        deeper_subsection_url = None
                                        try:
                                            deeper_link_tag = deeper_tag.find_element(By.XPATH, './/preceding::a[1]')
                                            if deeper_link_tag and deeper_link_tag.get_attribute('href'):
                                                deeper_subsection_url = deeper_link_tag.get_attribute('href').strip()
                                        except Exception as e:
                                            print(f"Error extracting URL for deeper subsection {deeper_subsection_title}: {e}")

                                        # Add deeper subsection to the current subsection
                                        subsection_data["subsubsections"].append({
                                            "title": deeper_subsection_title,
                                            "content": deeper_subsection_content,
                                            "subsubsection_url": deeper_subsection_url
                                        })

                                    # New block for handling level4 subsections
                                    ...
                                    # New block for handling level5 subsections
