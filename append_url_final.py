...
def update_urls(data, queries):
    ...
    for query in queries:
        ...
        base_url = f"https://codes.iccsafe.org/content/NYSFC2020P1/{query}"
...
def create_url(base_url, query, name, item_type):
    ...
    if "appendix" in query.lower():  # For Appendices
        ...
        if item_type == "chapter":
            ...
            return f"{base_url}#NYSECC2020P1_RE_Appx{appendix_identifier}"

        # Handle section format within appendix (e.g., SecA101.1)
        if item_type == "section":
            section_number = clean_name(name)  # Clean section number
            return f"{base_url}#NYSECC2020P1_RE_Appx{appendix_identifier}_Sec{appendix_identifier}{section_number}"
        ...
        if item_type == "subsection":
            ...
            return f"{base_url}#NYSECC2020P1_RE_Appx{appendix_identifier}_Sec{appendix_identifier}{section_number}.{subsection_number}"
        ...
        if item_type == "subsubsection":
            ...
            return f"{base_url}#NYSECC2020P1_RE_Appx{appendix_identifier}_Sec{appendix_identifier}{section_number}.{subsection_number}.{subsubsection_number}"
    ...
    if item_type == "chapter":
        return f"{base_url}#NYSFC2020P1_Pt01_{chapter_prefix}"
    ...
    if item_type == "section":
        ...
        return f"{base_url}#NYSFC2020P1_Pt01_{chapter_prefix}_Sec{section_number}"
    ...
    if item_type == "subsection":
        ...
        return f"{base_url}#NYSFC2020P1_Pt01_{chapter_prefix}_Sec{section_number}.{subsection_number}"
    ...
    if item_type == "subsubsection":
        ...
        subsection_number = clean_name(name.split(".")[1])  # Subsection part
        subsubsection_number = clean_name(name.split(".")[2])  # Subsubsection part
        return f"{base_url}#NYSFC2020P1_Pt01_{chapter_prefix}_Sec{section_number}.{subsection_number}.{subsubsection_number}"
